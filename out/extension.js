...
const addAliasesToShellConfig = (filePath) => {
    if (filePath) {
        fs.appendFile(filePath, `\n# Quick Aliases Extension\n${aliases.join("\n")}\n`, (err) => {
            if (err) {
                vscode.window.showErrorMessage(`Failed to update shell config file: ${err.message}`);
            }
            else {
                vscode.window.showInformationMessage("Aliases added to shell configuration. Please restart your terminal.");
            }
        });
    }
    else {
